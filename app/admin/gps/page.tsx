  ...
  return (
    <main className="px-6 py-8">
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-white">Partner GPs</h1>
          <p className="mt-1 text-sm text-white/45">
            {gps.length} GP{gps.length !== 1 ? "s" : ""} registered
          </p>
        </div>
        <Link
          href="/admin/gps/invite"
          className="rounded-xl bg-amber-500 px-4 py-2 text-sm font-semibold text-[#0f1729] transition-colors hover:bg-amber-400"
        >
          + Invite GP
        </Link>
      </div>
                  ...
                  <tr key={String(gp.id)} className="group transition-colors hover:bg-white/[0.03]">
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-3">
                        <div className="grid h-9 w-9 shrink-0 place-items-center rounded-full bg-amber-500/10 text-[11px] font-bold text-amber-300 ring-1 ring-amber-500/20">
                          {initials}
                        </div>
                        <div>
                          ...
                          <p className="text-xs text-white/40">{String(gp.email ?? "")}</p>
                        </div>
                      </div>
                    </td>
