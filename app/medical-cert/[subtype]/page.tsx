...
export default function MedicalCertIntakePage({
  params,
}: {
  ...
  async function handleSubmit() {
    ...
    if (travelDates.trim())
      notesParts.push(`Travel dates:\n${travelDates.trim()}`);
    if (destination.trim())
      notesParts.push(`Destination country:\n${destination.trim()}`);
    if (organisation.trim())
      notesParts.push(`Organisation / institution:\n${organisation.trim()}`);
          ...
          <div className="space-y-5 lg:col-span-2">
            <div>
              <label className="block text-sm font-medium text-[#0F2744]">
                Describe your reason for this certificate
            ...
            <TextInput
              label="Travel dates (if applicable)"
              value={travelDates}
              onChange={setTravelDates}
            />
            <TextInput
              label="Destination country (if applicable)"
              value={destination}
              onChange={setDestination}
            />
            <TextInput
              label="Organisation or institution requiring the cert"
              value={organisation}
